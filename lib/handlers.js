                        ...
                        _data.delete('check', checkId, function(err) {
                            ...
                            if(checksDeleted == checksToDelete) {
                                if(!deletionErrors) {
                                    ...
                                    callback(500, {Error: 'Errors encountered while attempting ot delete all of the users\'s checks. All of the checks many not have been deleted successfully'});
                                }
                            }
                        });
                    });
...
handlers._tokens.put = function(data, callback) {
  ...
  if (id && extend) {
        ...
        if (tokenData.expires > Date.now()) {
          ...
          _data.update("tokens", id, tokenData, function(err) {
            if (!err) {
              ...
              callback(500, {
                Error: "Could not update the token's expiraton"
              });
            }
          });
        } else {
    ...
    callback(400, {
      Error: "Missing required fields(s) or field(s) are invalid"
    });
  }
};
...
handlers._checks.post = function(data, callback) {
  ...
  var successCodes =
    ...
    data.payload.successCodes.length > 0
      ? data.payload.successCodes.trim()
      : false;
  var timeoutSeconds =
  ...
  if (id) {
    ...
    _data.read("checks", id, function(err, checkData) {
      if (!err && checkData) {
        ...
        var token =
          typeof data.headers.token == "string" ? data.headers.token : false;
        // Verify that the given token is valid and belongs ot the user who created the check
